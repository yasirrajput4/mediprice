import { Star, Loader2, SearchX, ChevronLeft, ChevronRight } from 'lucide-react';

// ── StarRating ────────────────────────────────────────────────────────────────
export function StarRating({ rating = 0, max = 5, size = 14, showValue = true }) {
  return (
    <span className="inline-flex items-center gap-1">
      ...
      {showValue && <span className="text-sm font-medium text-gray-700 ml-0.5">{Number(rating).toFixed(1)}</span>}
    </span>
  );
}
