...
export function BookingConfirmPage() {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-16 text-center">
      <div className={`w-20 h-20 rounded-full mx-auto flex items-center justify-center mb-5 ${isPaid ? 'bg-emerald-100' : 'bg-amber-100'}`}>
        <CheckCircle size={40} className={isPaid ? 'text-emerald-500' : 'text-amber-500'} />
      </div>
      ...
      <div className="card text-left space-y-3 mb-8">
        <div className="flex items-center gap-3 text-sm">
          ...
          <div>
            ...
            <div className="text-gray-500 text-xs">{booking.hospital_address}</div>
          </div>
        </div>
        <div className="flex items-center gap-3 text-sm">
          <Calendar size={16} className="text-blue-500" />
          <span className="text-gray-700">{new Date(booking.slot_date).toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</span>
        </div>
        <div className="flex items-center gap-3 text-sm">
          <Clock size={16} className="text-blue-500" />
          <span className="text-gray-700">{booking.slot_time?.substring(0,5)}</span>
        </div>
        <div className="border-t border-gray-100 pt-3 flex justify-between text-sm">
          <span className="text-gray-500">Total Paid</span>
          <span className="font-bold text-gray-900">₹{Number(booking.total_amount).toLocaleString('en-IN')}</span>
        </div>
        <div className="flex justify-between text-xs text-gray-400">
          <span>Booking ID</span>
          <span className="font-mono">{booking.uuid?.substring(0, 8).toUpperCase()}</span>
        </div>
      </div>
