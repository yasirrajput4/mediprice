...
export default function BookingPage() {
  ...
  const { data: slots = {} } = useQuery({
    ...
    queryFn: () =>
      hospitalsApi.slots(hospitalId, { service_id: serviceId, date: today }),
      ...
      <div className="flex items-center gap-2 mb-8">
        {["Select Slot", "Patient Details", "Payment"].map((s, i) => (
          <div key={s} className="flex items-center gap-2 flex-1">
            ...
            <span
              className={`text-xs font-medium hidden sm:block ${step === i + 1 ? "text-blue-700" : "text-gray-400"}`}
            >
              {s}
            </span>
            {i < 2 && (
              <div
                className={`flex-1 h-0.5 ${step > i + 1 ? "bg-emerald-400" : "bg-gray-200"}`}
              />
            )}
          </div>
        ))}
          ...
          {step === 3 && (
            <div className="card">
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm mb-5">
                ...
                <div className="flex justify-between text-gray-700">
                  <span>Mobile</span>
                  <span>+91 {form.patientPhone}</span>
                </div>
                <div className="flex justify-between text-gray-700">
                  <span>Date</span>
                ...
                <div className="flex justify-between text-gray-700">
                  <span>Time</span>
                  <span>{selectedSlot?.time?.substring(0, 5)}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2.5 text-xs text-emerald-700 mt-4">
                <Shield size={14} /> Secured by Razorpay · 256-bit SSL
                encryption
              </div>
          ...
          <div className="card sticky top-24">
            ...
            {selectedDate && selectedSlot && (
              <div className="mt-4 pt-4 border-t border-gray-100 space-y-2 text-xs text-gray-600">
                ...
                <div className="flex items-center gap-2">
                  <Clock size={13} />
                  {selectedSlot.time?.substring(0, 5)}
                </div>
                <div className="flex items-center gap-2">
                  <Clock size={13} />~{service?.wait_time_min} min wait
                </div>
              </div>
            )}
            {service?.preparation && (
