...
import { Search, MapPin, Clock, Shield, TrendingUp, ArrowRight, Star } from 'lucide-react';
import { servicesApi } from '../services/apiServices';
import { LoadingSpinner } from '../components/common/UI';

const POPULAR_SEARCHES = ['X-Ray', 'MRI', 'Blood Test', 'ECG', 'Ultrasound', 'CT Scan'];
    ...
    <div>
      ...
      <section className="bg-gradient-to-br from-blue-700 via-blue-600 to-teal-500 text-white py-16 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4 leading-tight">
            Compare Hospital Prices.<br />
            <span className="text-teal-200">Book Instantly.</span>
          </h1>
          <p className="text-blue-100 text-lg mb-8">
            Transparent pricing for X-rays, MRIs, blood tests and more — from verified hospitals near you.
          </p>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => navigate('/search')} className="text-sm text-blue-600 hover:underline flex items-center gap-1">
      ...
      <section className="bg-gray-50 py-12">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-xl font-bold text-gray-900">Trending in {city}</h2>
          </div>
          {trendingLoading ? (
              ...
              {trending.map((svc) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-gray-900 group-hover:text-blue-700 transition-colors text-sm truncate">{svc.name}</div>
                    <div className="text-xs text-gray-500 mt-0.5">{svc.hospital_count} hospitals</div>
                    <div className="flex items-center gap-3 mt-1.5">
                      <span className="text-sm font-bold text-blue-700">from ₹{Number(svc.lowest_price).toLocaleString('en-IN')}</span>
                      <span className="flex items-center gap-1 text-xs text-gray-400"><Clock size={11} />{svc.avg_wait_min} min wait</span>
                    </div>
                  </div>
                  <ArrowRight size={16} className="text-gray-300 group-hover:text-blue-500 transition-colors mt-1 flex-shrink-0" />
                </button>
              ))}
            </div>
          )}
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <h2 className="text-xl font-bold text-gray-900 text-center mb-10">How MediPrice Works</h2>
        <div className="grid md:grid-cols-3 gap-8">
          {[
            { step: '1', icon: <Search size={24} />, title: 'Search a Service', desc: 'Type any test or service — MRI, blood test, X-ray. We compare prices across nearby hospitals instantly.' },
            { step: '2', icon: <Star size={24} />, title: 'Compare & Choose', desc: 'See prices, wait times, ratings, and distance. Filter by budget or rating to find the best fit.' },
            { step: '3', icon: <Shield size={24} />, title: 'Book & Pay Securely', desc: 'Pick a slot, enter patient details, and pay online via Razorpay. Get instant confirmation.' },
          ].map((s) => (
            <div key={s.step} className="flex flex-col items-center text-center gap-3">
              <div className="w-14 h-14 rounded-2xl bg-blue-600 text-white flex items-center justify-center shadow-lg">
                {s.icon}
              </div>
              <div className="text-xs font-bold uppercase tracking-widest text-blue-500">Step {s.step}</div>
